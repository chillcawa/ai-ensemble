...
pub async fn chat(
    ...
    Ok(ChatResponseDto {
        ...
        input_tokens,
        output_tokens,
        cache_hit_input_tokens: value["usage"]["prompt_tokens_details"]["cached_tokens"]
            .as_u64()
            .map(|v| v as u32),
        cache_miss_input_tokens: None,
        cost_usd: None,
        pricing_basis: Some("xAI API / billed usage — verify in xAI Console".into()),
        truncated: false,
    })
}
...
pub async fn stream(
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|e| format!("xAI stream request failed: {e}"))?;
    if !response.status().is_success() {
        let status = response.status();
        let text = response.text().await.unwrap_or_default();
        return Err(format!("xAI stream error ({status}): {text}"));
    }
    ...
    let mut cache_hit_input_tokens = None;
    let mut process_line = |line: &str| {
        ...
        if let Ok(chunk) = serde_json::from_str::<Value>(payload) {
            ...
            if let Some(value) = chunk.get("usage").filter(|value| !value.is_null()) {
                ...
                cache_hit_input_tokens = value["prompt_tokens_details"]["cached_tokens"]
                    .as_u64()
                    .map(|v| v as u32);
            }
        }
    ...
    while let Some(chunk) = stream.next().await {
        let chunk = chunk.map_err(|e| format!("xAI stream read error: {e}"))?;
